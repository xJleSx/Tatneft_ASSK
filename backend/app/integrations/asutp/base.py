"""Интерфейс адаптера АСУ ТП.

Задел под будущую интеграцию (OPC-UA, REST). В текущей версии прототипа
реальный адаптер не подключён — данные в БД попадают через seed-скрипт
(`app.mocks.generators.seed.seed_telemetry_history`).
